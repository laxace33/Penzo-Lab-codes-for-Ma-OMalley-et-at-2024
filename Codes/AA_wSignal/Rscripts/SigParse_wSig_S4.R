...
####################these lines may be specific to Jun's experiments but I left them in for now
...
sdir=paste(root,"rawsig",sep="")
sfiles=list.files(sdir)

tdir=paste(root,"rtrack",sep="")
tfiles=list.files(tdir)

pdir=paste(root,"rawsig_ttl",sep="")
pfiles=gsub("CUE","cue",list.files(pdir))
pfiles=gsub("SHOCK","shock",pfiles)

figdir=paste(root,"figsout",sep="")
...
#dat$event[which(dat$shock==1)]="shock"
...
gf=read.csv(paste(sdir,sfiles [match(paste (paste(tmatch,"GC",sep="_"),".csv",sep=""),smatch)],sep="/"),stringsAsFactors=F)
gf=gf[,c(grep("TIME",colnames(gf)),grep("D0",colnames(gf)))]
colnames(gf)=c("time","val")

af=read.csv(paste(sdir,sfiles [match(paste (paste(tmatch,"AF",sep="_"),".csv",sep=""),smatch)],sep="/"),stringsAsFactors=F)
af=af[,c(grep("TIME",colnames(af)),grep("D0",colnames(af)))]
colnames(af)=c("time","val")

pulse=read.csv(paste(pdir,pfiles [match(paste (paste(tmatch,"cue",sep="_"),".csv",sep=""),plmatch)],sep="/"),stringsAsFactors=F)
ptimes=pulse[,c(grep("TIME",colnames(pulse)),grep("D0",colnames(pulse)))]


if (nrow(ptimes)>1000) ({
rawpulse=ptimes$D0
rawpint=findInterval(rawpulse,c(0,1,3,4))
rlepulse=rle(rawpint)
cumlength=cumsum(rlepulse$lengths)
realpulse=which(rlepulse$values==3)-1

ptimes=ptimes$TIME [cumlength[realpulse]+1]
}) else ({
ptimes=ptimes[,grep("TIME",colnames(ptimes))]
})

first_spulse=ptimes[1]
first_tpulse=dat$time [rle(dat$cues)$lengths[1]+1]

diffp=first_spulse-first_tpulse

newtime=dat$time+diffp

dat$time=newtime

pcum=cumsum((rle(dat$cues)$lengths))+1
am_pulse=dat$time [pcum [which (rle(dat$cues)$values==1)-1]]

rpulse=data.frame (cbind(ptimes,am_pulse),stringsAsFactors=F)
rpulse$difft=rpulse$ptimes-rpulse$am_pulse

sig=gf
colnames(sig)=c("time","gc")

sig$af=af$val

getvals.rfunc=function(x) ({
v1=x
forx=seq (1,nrow(v1))
v2=lm (v1$val~forx)
coeffA=coef(v2)

fitforaf=((coeffA[2])*forx) + coeffA[1] 
subAF=v1$val-fitforaf
out=list()
out[[1]]=subAF
out[[2]]=as.vector (coeffA[1])

return(out)
})


afcor=getvals.rfunc(af) #get slope and y intercept for auto fluor
gcor=getvals.rfunc(gf)  #get slope and y intercept for gcamp

afnew=afcor[[1]]+ gcor[[2]]  #
gfnew=gcor[[1]]+ gcor[[2]]
...
getvidframe.rfunc=function(x,y) ({
tvec=as.vector(unlist(y))
event0=which.min (abs (x[,1]-tvec))
return(event0)
})

sig$vidtime=dat$time[sapply (seq(1,nrow(sig)),function(p) (getvidframe.rfunc(sig[p,],dat$time)))] # align sig time to video time i.e. time bin like approach
sig$vid_fnum=match(sig$vidtime,dat$time)
...
esc=match(cuedf$esc_start,round (dat$time,digits=3))[eidx]
...
sig$tint=findInterval(sig$time,cuedf$start)

formax=cbind (cuedf$f_start,cuedf$esc_frame)

if (length(which(cuedf$shock=="yes"))>0) ({
formax[which(cuedf$shock=="yes"),2]=cuedf$f_shock_start[which(cuedf$shock=="yes")]-1
})
...
itisig=apply(itibind,1,function(x) (which(sig$time>x[1]) [which(sig$time>x[1]) %in% which(sig$time<x[2])]))


dat$iti [unlist(ititrack)]="iti"
dat$iti [which(dat$tint==0)]="iti"

sig$iti [unlist(itisig)]="iti"
sig$iti [which(sig$tint==0)]="iti"
...
################################################################################################################################################################
sig$val[1:10]=mean(sig$val[11:20])

sig$fscale_all=(2 *(sig$val-min(sig$val,na.rm=T))/(max(sig$val,na.rm=T)-min(sig$val,na.rm=T)))-1

sig$snum=seq(1,nrow(sig))
splitevent=split(sig,sig$tint)
tbaseline=lapply(splitevent,function(x) (x$val[which(x$iti=="iti")]))

if(length(tbaseline)>nrow(cuedf)) ({
tbaseline=tbaseline[-length(tbaseline)]
})


sps=mean(diff(sig$time))

tbaseline=lapply (tbaseline,function(x) (x[(length(x)-round (1/sps)):length(x)]))

bmean=lapply(tbaseline,function(x) (mean(x,na.rm=T)))
sdbase=lapply(tbaseline,function(x) (sd(x,na.rm=T)))

if (length (which (is.na (unlist(bmean))))>0) ({
rmna=which (is.na (unlist(bmean)))
bmean=bmean[-rmna]
sdbase=sdbase[-rmna]
})


idx=as.list (seq(2,length(splitevent)))
zscore=lapply(idx,function(x) ((splitevent[[x]]$val-bmean[[x-1]])/sdbase[[x-1]])) # subtract trial by trial baseline

#############################this was changed for the second submission
zscore=c((splitevent[[1]]$val-bmean[[1]])/sdbase[[1]],unlist (zscore))
#############################this was changed for the second submission


zidx=as.vector (unlist (lapply(splitevent,function(x) (x$snum))))
sig$z_base[zidx]=unlist(zscore)

newsplit=split(sig,sig$tint)

scaleidx=as.list (seq(1,length(newsplit)))
fscale_base=lapply(scaleidx,function(x)((2 *(newsplit[[x]]$val-min(newsplit[[x]]$val))/(max(newsplit[[x]]$val)-min(newsplit[[x]]$val)))-1))
sig$fscale_base=unlist (fscale_base)    

sig$bevent=NA
sig$event=NA
sig$loco_event=NA
sig$sub_thresh_freeze=NA

sig$event[unlist (apply(cuedf[,c(1,2)],1,function(x) (which(sig$time>=x[1]) [which(sig$time>=x[1]) %in% which(sig$time<=x[2])])))]="cue"
sig$event[unlist (apply(cuedf[,c(7,8)],1,function(x) (which(sig$time>=x[1]) [which(sig$time>=x[1]) %in% which(sig$time<=x[2])])))]="shock"

sig$bevent[unlist (apply(fdat[,c(1,2)],1,function(x) (which(sig$time>=x[1]) [which(sig$time>=x[1]) %in% which(sig$time<=x[2])])))]="freeze"

sig$bevent[unlist (apply(fdat[which(fdat$cue=="yes"),c(1,2)],1,function(x) (which(sig$time>=x[1]) [which(sig$time>=x[1]) %in% which(sig$time<=x[2])])))]="cue_freeze"
sig$bevent[unlist (lapply (as.list (cuedf$esc_start),function(x) (which.min (abs (x-sig$time)))))]="esc"


if (is.null (nrow(subthresh_freeze))==FALSE) ({
sig$sub_thresh_freeze[unlist (apply(subthresh_freeze[,c(1,2)],1,function(x) (which(sig$time>=x[1]) [which(sig$time>=x[1]) %in% which(sig$time<=x[2])])))]="freeze"
})


sig$loco_event[unlist (lapply (as.list (cuedf$emov),function(x) (which.min (abs (x-sig$time)))))]="esc_start"
sig$loco_event[unlist (lapply (as.list (cuedf$maxtime),function(x) (which.min (abs (x-sig$time)))))]="max_vel"

sig$event [which(sig$iti=="iti")]="iti"

dat$vel_cm=round (dat$dists * (1/fps[2]) *0.1,digits=2)
vidx=c(seq(0,20,0.25),1000)

vint=findInterval(dat$vel_cm,vidx)
pcol=matlab.like(82)
dat$vcol=pcol[vint]

sig$dnum=unlist (lapply (as.list (sig$time),function(x) (which.min (abs (x-dat$time)))))

sig$vel_cm=dat$vel_cm[sig$dnum]
sig$vcol=dat$vcol[sig$dnum]


plot.rfunc=function(x,s) ({

rsig=range(x$val,na.rm=T)
yrange=seq(-1000,1000,10)
yint=findInterval(rsig,yrange)
yint[2]=yint[2]+1


par (pty="s")
plot.new()
plot.window(xlim=c(1,nrow(x)),ylim=c(yrange [yint[1]],yrange [yint[2]]))

abline (h=yrange [yint[1]],lty=2,lwd=0.25,col="light gray")
abline (h=yrange [yint[2]],lty=2,lwd=0.25,col="light gray")
abline (h=0,lty=2,lwd=0.25,col="light gray")

par(las=1)
par(adj=1)

mtext(yrange [yint[1]],2,0,yrange [yint[1]],line=1)
mtext(yrange [yint[2]],2,0,yrange [yint[2]],line=1)
mtext(0,2,0,0,line=1)

par(las=3)
par(adj=1)
mtext("Corrected Raw Signal",2,line=1)


lines(seq (1,nrow(x)),x$val,col=" light gray",type="l")
points (which(x$event=="cue"), x$val[which(x$event=="cue")],col="orange",cex=0.5,,pch=19)
points (which(x$event=="shock"), x$val[which(x$event=="shock")],col="firebrick1",cex=0.5,,pch=19)
points (which(x$bevent=="freeze"), x$val[which(x$bevent=="freeze")],col="mediumpurple",pch=19,cex=0.5)
points (which(x$bevent=="cue_freeze"), x$val[which(x$bevent=="cue_freeze")],pch=19,col="midnightblue",cex=0.5)
#points (which(x$loco_event=="esc_start"), x$val[which(x$loco_event=="esc_start")],col="black",pch=1,cex=1)
points (which(x$sub_thresh_freeze=="freeze"), x$val[which(x$sub_thresh_freeze=="freeze")],pch=19,col="dodgerblue",cex=0.5)


points (which(x$bevent=="esc"), x$val[which(x$bevent=="esc")],col="turquoise3",pch=1,cex=1)
points (which (x$loco_event=="esc_start"), x$val[which (x$loco_event=="esc_start")],col="seagreen3",pch=19,cex=1)

points (which (x$loco_event=="max_vel"), x$val[which (x$loco_event=="max_vel")],col="pink",pch=19,cex=1)

#points (which (x$loco_event=="esc_start"), x$val[which (x$loco_event=="esc_start")],col="seagreen3",pch=19,cex=1)
outname=paste (s,paste("trial",x$tint[1],sep="_"),sep=" ")

par(las=1)
par(adj=0.5)
mtext(outname,3)

colidx=c("orange","firebrick1","mediumpurple","midnightblue","turquoise3","seagreen3","pink","dodgerblue")
legend("topright",legend=c("cue","shock","freeze","cue_freeze","esc","esc_start","max_vel","sub_thresh_freeze"),border=NULL,bty="n",pch=c(rep(19,4),1,rep(19,2)),cex=0.75,col=colidx,inset=0)
#legend(1,legend=c("cue","shock","freeze","cue_freeze","esc","esc_start"),border=NULL,pch=c(rep(19,4),1,rep(19,2)))

#points(seq(1,nrow(x)),rep(yrange [yint[2]],nrow(x)),col=x$vcol,pch=19)

rectdims= (yrange [yint[2]]-yrange [yint[1]])*0.05
rect(seq(1,nrow(x)),yrange [yint[1]]-rectdims,seq(1,nrow(x))+0.5,yrange [yint[1]], border = NA,col=x$vcol)
...
pdfname=paste (inf,".pdf",sep="")
pdf(paste(figdir,pdfname,sep="/"))

forplot=split(sig,sig$tint)
toplot=forplot [match(cuedf$tint,as.numeric (names(forplot)))]

lapply(toplot,function(p) ({plot.rfunc(p,inf)}))

graphics.off()


csvname=paste (inf,".csv",sep="")
getdist=dat[which(dat$event=="cue"),]
distsplit=split(getdist,getdist$tint)

cuedf$cue_dist=NA
cuedf$cue_dist[as.vector (unlist (lapply(distsplit,function(x) (x$tint[1]))))]=round (as.numeric (unlist (lapply(distsplit,function(x) (sum(x$dists,na.rm=T))))),digits=2)
...
save (sigout,file=paste (paste (root,"rout",sep=""),"sigout.rdat",sep="/"))
write.csv(pointer,file=paste(root,"pointer/finalpointer.csv",sep=""),row.names=F)
